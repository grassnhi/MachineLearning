...
class HOLDING(PREDICATE):
    ...
    def get_action(self, world_state):
        ...
        if ONTABLE(X) in world_state:
            return PickupOp(X)
        elif any(isinstance(predicate, ON) and predicate.X == X for predicate in world_state):
            on_predicate = next(predicate for predicate in world_state if
                               isinstance(predicate, ON) and predicate.X == X)
            return UnstackOp(X, on_predicate.Y)
        else:
            return None
        ...
        while len(stack) != 0:
            ...
            if type(stack_top) is list:
                ...
                if action is not None:
                    ...
                    print("No applicable action found for goal:", unsatisfied_goal)
